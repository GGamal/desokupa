<template>
  <div class="q-pa-md">
    ...
    <q-table
      ...
      no-data-label="No hay contratos asignados"
      no-results-label="No hay contratos asignados"
    >
      ...
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <div class="row no-wrap items-center justify-center">
            <q-icon name="circle" size="sm" :color="props.row.status === 5 ? 'yellow' : ''" />
            <div>{{props.row.status === 5 ? 'Asignado' : ''}}</div>
          </div>
        </q-td>
      </template>
      <template v-slot:body-cell-opcion="props">
        <q-td :props="props">
          <div class="row no-wrap justify-center q-gutter-xs">
            ...
            <q-btn v-if="props.row.status === 5" label="Legal" icon="flag" color="blue" size="sm" dense rounded push no-wrap style="width:90px"
            ...
            <q-btn v-if="props.row.status === 5" label="Finalizar" color="green" size="sm" dense rounded push no-wrap style="width:90px"
    ...
    <q-dialog v-model="newHito">
      <q-card class="q-pa-none" style="width:100%">
        ...
        <div class="q-px-lg q-pb-xl">
          ...
          <q-btn style="width:100%" no-caps :label="status === 6 ? 'Disponible para Legal' : 'Finalizar contrato'" color="primary" size="lg"
          @click="generar()" />
        </div>
      </q-card>
    </q-dialog>
...
export default {
  ...
  methods: {
    getContratos () {
      ...
      this.comercios = []
      this.$api.get('contratos_asignados').then(res => {
        if (res) {
          this.allData = res
