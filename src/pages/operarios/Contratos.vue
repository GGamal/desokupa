      ...
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <div class="row no-wrap items-center justify-center">
            <q-icon name="circle" size="sm" :color="props.row.status === 4 ? 'blue' : props.row.status === 5 ? 'yellow' : ''" />
            <div>{{props.row.status === 4 ? 'Disponible' : props.row.status === 5 ? 'Asignado a OP' : ''}}</div>
          </div>
        </q-td>
      </template>
      <template v-slot:body-cell-opcion="props">
        <q-td :props="props">
          <div class="row no-wrap justify-center q-gutter-xs">
            ...
            <q-btn v-if="props.row.status === 4" label="Asignar" icon="flag" color="blue" size="sm" dense rounded push no-wrap style="width:90px"
    ...
    generar () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.$api.post('generar_hito', formData, {
          ...
          if (res) {
            this.$api.put('status_contrato/' + this.dataFormulario._id, { operario_id: this.form.operario_id, status: 5 }).then(v => {
              if (v) {
                this.getContratos()
                this.newHito = false
                this.$q.notify({
