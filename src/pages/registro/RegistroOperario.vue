<template>
  <div class="q-pa-lg">
    ...
    <div class="q-my-lg text-center text-h5 text-bold text-grey-8">{{!edit ? 'REGISTRO DE OPERARIO' : 'EDICIÓN DE OPERARIO'}}</div>
  ...
  methods: {
    ...
    registrar () {
      ...
      if (!this.$v.provincia.$error && !this.$v.localidad.$error && !this.$v.form.$error && !this.$v.password.$error && !this.$v.repeatPassword.$error) {
        ...
        this.form.roles = [5]
        this.form.lider_id = this.user._id
        ...
        this.$api.post('register', this.form).then(res => {
          if (res) {
            this.$q.notify({
              message: 'Operarios registrado con éxito',
              color: 'positive'
            })
            this.$q.loading.hide()
            this.$router.go(-1)
