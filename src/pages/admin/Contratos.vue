<template>
  <div class="q-pa-md">
    <div>
      <div class="text-h5 text-grey-10 text-weight-medium">Listado de contratos</div>
      <div class="text-grey-9">
        {{`En la siguiente tabla puedes ver todos los contratos vigentes\n
        en el sistema. Recuerda que en la columna Acción puedes\n
...
export default {
  ...
  methods: {
    getContratos () {
      ...
      this.comercios = []
      this.$api.post('contratos_by_status', { status: [2, 4, 5, 6, 7, 8] }).then(res => {
        if (res) {
          this.allData = res
