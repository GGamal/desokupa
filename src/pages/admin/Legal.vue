<template>
  <div class="q-pa-md">
    <div>
      <div class="text-h5 text-grey-10 text-weight-medium cursor-pointer" @click="filtrar(null), tab = null">Todos los contratos disponibles</div>
      <div class="text-grey-9">
        {{`La siguiente tabla muestra todos los contratos con un problema legal activo. \n
        Recuerda que en la columna Acción puedes\n
        precionar el botón Ver, para ver el historial del contrato.`}}
      </div>
    </div>
...
export default {
  ...
  methods: {
    getContratos () {
      ...
      this.comercios = []
      this.$api.post('contratos_by_status', { status: [6, 7] }).then(res => {
        if (res) {
          this.allData = res
