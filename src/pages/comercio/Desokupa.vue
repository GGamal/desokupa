<template>
  <div class="q-pa-md">
    <div>
      <div class="text-h5 text-grey-10 text-weight-medium cursor-pointer" @click="filtrar(null), tab = null">Presupuestos Desokupa</div>
      <div class="text-grey-9">
        {{`En la siguiente tabla puedes revisar los presupuestos de tipo Desokupa\n
    ...
    >
      <template v-slot:body-cell-type="props">
        <q-td :props="props">
          <div class="text-center">Contrato Desokupa</div>
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-opcion="props">
        <q-td :props="props">
          <div class="row no-wrap justify-center q-gutter-xs">
            <q-btn icon="visibility" color="orange" size="sm" label="Ver" dense rounded push no-wrap style="width:90px"
            @click="ver(props.row)"/>
            <q-btn v-if="props.row.status === 0" label="Aprobar" icon="thumb_up_off_alt" color="green" size="sm" dense rounded push no-wrap style="width:90px"
    ...
    <q-dialog v-model="dialog">
      <q-card class="q-pa-none" style="width:100%">
        ...
        <div class="text-grey-8 text-center text-h5 q-mt-md">Generar link</div>
        <div class="q-px-lg q-pb-xl">
          ...
          <div v-if="form.type === 1">
            Valor
            <q-input type="number" outlined v-model.number="form.valor"  dense placeholder="Valor del contrato"
          ...
          <div>
            Cliente
            <q-select outlined dense color="black" v-model="cliente" :options="clientes" label="Seleccione el cliente" map-options
...
export default {
  ...
  methods: {
    getContratos () {
      ...
      this.$api.get('contratos/1').then(res => {
        if (res) {
          this.allData = res
