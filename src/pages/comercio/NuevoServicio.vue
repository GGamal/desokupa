        ...
        <div class="q-px-lg q-pb-xl">
          ...
          <div>
            ...
            <q-select outlined dense color="black" v-model="form.categoria" :options="categorias" label="Seleccione una categoria" map-options
              ...
              option-label="name" option-value="val" emit-value @input="verify(form.categoria)">
            </q-select>
          </div>
          <div>
            Subcategoria
            <q-select outlined dense color="black" v-model="form.categoria" :options="categorias" label="Seleccione una subcategoria" map-options
              error-message="Este campo es requerido" :error="$v.form.categoria.$error" @blur="$v.form.categoria.$touch()"
              option-label="name" option-value="val" emit-value @input="verify(form.categoria)">
            </q-select>
          </div>
...
export default {
  data () {
    return {
      ...
      categorias: [
        { val: 1, name: 'Dev Apps' },
        { val: 2, name: 'Marketing' },
        { val: 3, name: 'Hosting' },
        { val: 4, name: 'Servicio IT' }
      ]
    }
  },
  validations: {
