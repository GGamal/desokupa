<template>
  <div class="q-pa-md">
    <div class="row justify-between items-center q-py-md">
      <div class="text-h6 cursor-pointer">Lista de Todos mis Contratos</div>
      <div class="row">
    ...
    <q-dialog v-model="newHito">
      <q-card class="q-pa-none" style="width:100%">
        ...
        <div class="q-px-lg q-pb-xl">
          ...
          <q-btn style="width:100%" no-caps :label="status === 5 ? 'Disponible para Legal' : 'Finalizar contrato'" color="primary" size="lg"
          @click="generar()" />
        </div>
      </q-card>
    </q-dialog>
...
export default {
  data () {
    return {
      ...
      statuses: [
        { val: 5, name: 'Asignado a Operario' },
  ...
  methods: {
    getContratos () {
      ...
      this.comercios = []
      this.$api.post('my_contracts', { status: [5, 8] }).then(res => {
        if (res) {
          this.allData = res
          ...
          if (res) {
            this.$api.put('status_contrato/' + this.dataFormulario._id, { status: this.status }).then(v => {
              if (v) {
                ...
                this.newHito = false
                this.$q.notify({
                  message: 'Tarea generado con éxito',
                  color: 'positive'
                })
                this.$q.loading.hide()
              } else {
